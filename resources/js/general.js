...
export const formatDate = (date = null, format = null) => {
    ...
    // 1. Lógica aprimorada para criar o objeto Date corretamente
    if (typeof date === 'string') {
        // Regex para verificar se a string é APENAS data (YYYY-MM-DD)
        const dateOnlyRegex = /^\d{4}-\d{2}-\d{2}$/;

        if (dateOnlyRegex.test(date)) {
            // Se for apenas data, evitamos a conversão indesejada de fuso horário.
            // Dividimos a string e passamos os componentes para o construtor Date.
            // Isso força o JS a tratar a data como local, não UTC.
            ...
            // Se for data e hora (ou outro formato), usamos o método anterior que funciona bem.
    ...
    // 2. A partir daqui, a lógica de formatação permanece a mesma,
    // pois 'currentDate' sempre estará correto e no fuso horário local.
